...
### Canonical Chapter Numbering (TASK-014)

The `chapters` field in Supabase uses this fixed numbering. Source of truth: `chapter_sections` table in Supabase. **Do not use the old numbering (pre-2026-03-22 remapeo).**
...
- **Cap 1 (Introducción):** Background motivation, problem statement, scope of thesis
- **Cap 2 (Marco Teórico):** AI governance theory, regulatory frameworks, comparative law methodology, right to explanation, human oversight theory
- **Cap 3 (Estado del Arte):** Existing literature, prior work, empirical studies on AI Act / NIST RMF / AI governance
- **Cap 4 (Metodología):** Research design, functional equivalence analysis, comparative methodology
- **Cap 5 (Análisis Comparativo):** Direct EU AI Act ↔ NIST RMF article-by-article comparisons, interoperability analysis, gap analysis
- **Cap 6 (Caso de Estudio):** Concrete cases, court rulings, empirical examples (forensic AI, STRmix, PREDPOL)
- **Cap 7 (Conclusiones):** Policy recommendations, synthesis, future research

**Format:** JSON integer array, e.g., `[2, 3]`, `[5]`, `[2, 5]`. Never leave empty — assign at minimum one chapter. If genuinely cross-cutting, list all relevant chapters.
...
2. **Create `SKILL-KB-vN+1.md`** in `phd-kb/docs/` with all changes applied. Current version is v18, so next update produces `SKILL-KB-v19.md`.

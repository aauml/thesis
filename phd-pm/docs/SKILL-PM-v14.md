...
### Columna `chapters` (integer[]) — IMPLEMENTADO 2026-03-22

Columna `chapters` activa en `evaluated_items`. Mapea cada item a capítulos de la tesis:
- Cap 1: Introducción (EU AI Act, NIST AI RMF, interoperabilidad)
- Cap 2: Metodología
- Cap 3: Marco teórico (regulación AI, riesgo, gobernanza algorítmica)
- Cap 4: Estado del Arte (revisión de literatura)
- Cap 5: Análisis comparativo (EU vs US, mapeo artículos)
- Cap 6: Caso de estudio forense (STRmix, PG, biometría)

**Distribución actual (1,495 items):** Cap1=748, Cap2=37, Cap3=726, Cap4=818, Cap5=574, Cap6=378.

**Consulta por capítulo:** `SELECT title, scholar, importance FROM evaluated_items WHERE 5 = ANY(chapters) ORDER BY importance;`

**Matriz de cobertura:** `SELECT unnest(chapters) as cap, importance, count(*) FROM evaluated_items GROUP BY 1,2 ORDER BY 1,2;`

### Protecciones de datos — IMPLEMENTADO 2026-03-22

- **Trigger auto-corrección** (`trg_validate_evaluated_item`): corrige campos incompletos en INSERT/UPDATE (importance→MEDIA, scholar→[sin asignar], chapters→{4}, thesis_relevance→[PENDIENTE]). Solo rechaza title/url vacíos. Correcciones en `kb_corrections_log`.

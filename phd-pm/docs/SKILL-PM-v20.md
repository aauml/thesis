...
**Relación con phd-kb:**
- phd-kb gestiona: index.html, SKILL-KB, pipelines de datos del Knowledge Base
- phd-pm gestiona: dashboard.html, PM-SessionLog, decisiones de investigación, Obsidian vault, **triaje de colas de staging** (§19)
- Compartido: **scripts GAS** (`phd-kb/gas/`) — ambos proyectos pueden editar y desplegar vía CLASP (§20). **Supabase** (primario, lectura + escritura en `reading_plan`, `pm_advisories`, `reading_conversations`, y `evaluated_items`) + Sheet API (staging queues + verificación)
- **Connectors (Claude.ai):** Scholar Gateway (búsqueda semántica académica), Consensus (papers con señales de acuerdo científico), Google Calendar (deadlines/reuniones). El PM usa estos connectors como herramientas dentro del flujo de trabajo — no son pipelines independientes. Resultados pasan por el mismo proceso de evaluación que cualquier otra fuente.
- Regla: PM no modifica index.html sin coordinar con KB. KB no modifica dashboard.html. PM lee `SKILL-KB-current.md` del repo para el protocolo de evaluación — si KB actualiza ese archivo, PM hereda los cambios automáticamente.
...
├── phd-kb/
│   ├── gas/                      # GAS source of truth (CLASP CI/CD, §20)
│   │   ├── .clasp.json           # Script ID binding
│   │   └── *.js                  # WebApp, orchestrators, pipelines
│   ├── scripts/                  # Archivo histórico pre-CLASP (no editar)
...
Pushear directamente sin pedir confirmación para commits al Dashboard y scripts GAS (`phd-kb/gas/`). Sí pedir confirmación para cambios a index.html o SKILL-KB.
...
6b. **Connector task pickup (pm_tasks):**
```sql
SELECT id, title, description, due_date
FROM pm_tasks
WHERE task_type = 'busqueda' AND done = false
AND (description ILIKE '%Scholar Gateway%' OR description ILIKE '%Consensus%')
ORDER BY due_date ASC NULLS LAST;
```
If pending connector tasks exist: note them in the readiness report. Execute during the session if user approves or if flagged as priority. If no pending tasks, skip — connectors are demand-driven, not mandatory every session.

7. **KB queue check (staging queues):**
```bash
curl -s -L "SHEET_API?action=getStats"
```
Report pending counts: "Queues: Perplexity X | Academic Y | News Z." If total > 0 and user requested `update all` or explicit queue processing, execute §19 after startup completes. Otherwise note pending count and move on.

8. **Confirm readiness:**
Report: "PM lessons read (N rules). CHANGELOG: [resumen últimas entradas o 'sin cambios']. Advisories due: [N due / none]. Queues: [counts]. Last PM session: [date]. Pending: [items]. Ready."
...
7b. **Connector tasks:** If during the session the PM identified knowledge gaps that warrant connector searches but didn't execute them (time, context limits), create `pm_tasks` entries:
```sql
INSERT INTO pm_tasks (title, description, task_type, review_status)
VALUES (
  'Scholar Gateway: [gap topic]',
  '[Scholar Gateway] Search for: [specific query]. Gap: [which gap]. Chapter: [N]. Expected: [what we hope to find].',
  'busqueda',
  'none'
);
```
This ensures connector work is queued, visible in the dashboard, and picked up in the next session.
...
### Estructura canónica de capítulos — ACTUALIZADO 2026-03-22 (v2: estructura híbrida)

**Fuente única de verdad:** tabla `chapter_sections` en Supabase (32 secciones en 7 capítulos).

| Cap | Título | Secciones | KB sources | ALTA | Lecturas asignadas |
|-----|--------|-----------|------------|------|-------------------|
| 1 | Introducción | 5 | 0 | 0 | 0 |
| 2 | Marco teórico y estado de la cuestión | 6 | 528 | 117 | 16 |
| 3 | El Reglamento de Inteligencia Artificial | 5 | 287 | 101 | 6 |
| 4 | El NIST AI Risk Management Framework | 4 | 389 | 117 | 7 |
| 5 | Análisis comparativo | 4 | 38 | 19 | 8 |
| 6 | Caso de estudio: Departamento de Justicia | 4 | 164 | 58 | 7 |
| 7 | Conclusiones | 4 | 0 | 0 | 1 |

**Nota:** KB limpiada 2026-03-22. De 1,495 → 1,332 registros (163 duplicados eliminados, 27 author-names retitulados, 19 capas normalizadas). Reasignación con clasificador v2 (title-first, max 2 caps/item). 94.4% single-chapter.

**Vista `chapter_coverage`:** cruza chapter_sections + evaluated_items.chapters + reading_plan.chapter_ids en tiempo real. El dashboard la consume directamente (0 hardcode).

**`reading_plan.chapter_ids`** (integer[]): reemplaza el campo texto `chapters`. Permite joins directos con `evaluated_items.chapters` y `chapter_sections.chapter_num`.

**Consulta por capítulo:** `SELECT title, scholar, importance FROM evaluated_items WHERE 5 = ANY(chapters) ORDER BY importance;`

**Vista de cobertura completa:** `SELECT * FROM chapter_coverage;`

**REGLA:** Cualquier cambio en la estructura de capítulos se hace SOLO en `chapter_sections`. Dashboard, skills, y advisory leen de ahí dinámicamente.

### Protecciones de datos — ACTUALIZADO 2026-03-22

- **Trigger validación estricta** (`trg_validate_evaluated_item`): rechaza items sin title (≥5 chars), url (≥10 chars), importance (ALTA/MEDIA/BAJA), thesis_relevance (≥100 chars, sin patrones degradados), scholar (≥2 chars), chapters vacíos, **chapters fuera de rango 2-6**, o **más de 3 chapters por item**. Ch1 (Intro) y Ch7 (Conclusiones) no reciben fuentes. El array `chapters` sigue orden de prioridad: posición 1 = primario, 2 = secundario, 3 = terciario.
- **Índices:** GIN en `chapters` (queries `ANY(chapters)`), B-tree en `importance`.
- **Health check diario** (`kb-daily-health-check`, 7am UTC): valida títulos, importance, thesis_relevance, scholar, embeddings, capítulos inválidos, items con 3 caps (info), duplicados. Resultados en `kb_health_log`.
- **Health check** (`kb_health_check()`): 13 checks de calidad. Cron diario 7am UTC → `kb_health_log`.
- **Pipeline documentada** en `phd-kb/docs/PIPELINE-ARCHITECTURE.md`.
...
**Claude.ai Connectors:** Scholar Gateway (semantic academic search), Consensus (scientific agreement signals), Google Calendar (schedule management). Connected 2026-03-24.
...
- Modificar index.html o SKILL-KB sin coordinar con phd-kb
...
- Usar Scholar Gateway o Consensus sin registrar los resultados en el pipeline de evaluación completo (importance, capa, chapters, thesis_relevance)
- Crear items en evaluated_items desde connectors sin crear primero el pm_task que documenta qué se buscó y por qué
- Usar Google Calendar para crear eventos sin confirmación del doctorando
...
| `check_count` | int | Checks sin resultado. Se resetea a 0 cuando un check encuentra algo. Determina nivel de escalamiento (ver tabla). |

### Ciclo de seguimiento

**En Session Startup (paso 5):**

```sql
SELECT id, title, status, next_review, review_interval, search_queries, check_notes
FROM pm_advisories 
...
2. **Evaluar resultados y actuar** — no solo reportar:
   - Si hay hallazgo: actualizar advisory, recomendar acción concreta (agregar al plan, abrir nota, cambiar queries)
   - Si no hay hallazgo: **cambiar algo** antes de mover la fecha. Ver regla de escalamiento abajo.
3. Actualizar `last_checked`, `check_notes`, `next_review`
4. Reportar al doctorando solo lo que requiere su atención o decisión

**Regla de escalamiento — "no repetir la misma búsqueda sin cambio":**

Cuando un check no produce resultados, el PM **debe** hacer al menos una de estas cosas antes del siguiente ciclo:

| Check sin resultado | Acción del PM |
|--------------------|----|
| 1er check vacío | Reformular queries: sinónimos, otros idiomas, ampliar scope |
| 2do check vacío | Cambiar fuentes: probar otra base de datos, otro tipo de búsqueda (web → scholar, keyword → semántica), o buscar autores específicos que publican en el tema |
| 3er check vacío | Proponer estrategia alternativa: ¿el gap se puede cubrir de otro modo? ¿Hay fuentes tangenciales que sirvan? ¿Conviene reclasificar el target? Documentar recomendación en `check_notes` |
| 4+ checks vacíos | Justificar y ajustar: explicar por qué el gap persiste (ej: campo poco publicado, timing, idioma), proponer target revisado o reclasificar a `parked` con justificación |

Ejemplo concreto — Gap procesal español (#3):
- Check 1 (queries actuales): "prueba electrónica IA", Bueno de Mata → 0 nuevos
- Check 2: reformular a "algoritmo proceso judicial España", probar Dialnet, CENDOJ, vLex → si 0, anotar
- Check 3: buscar autores del círculo de Bueno de Mata, tesis doctorales recientes en TESEO, actas de congresos procesales → si 0, proponer: "Este gap puede requerir captura manual — el campo es pequeño en español. Propongo: (a) revisar bibliografía de los papers que sí tenemos para encontrar citados, (b) ampliar a derecho procesal europeo comparado como puente"
- Check 4: si sigue vacío, proponer ajustar target de 15 a 10, o reclasificar parte del gap como cubierto por fuentes en otros idiomas que tratan el caso español

**El PM no es pasivo.** El seguimiento no es "busqué y no encontré, nos vemos en 2 semanas." Es "busqué, no encontré, modifiqué X para la próxima vez y propongo Y como alternativa."
...
- **El PM es dueño del seguimiento, no el doctorando.** El doctorando no tiene que preguntar "¿qué pasó con X?" — el PM lo trae proactivamente.
- **Intervalos adaptativos:** El PM ajusta frecuencia según contexto político y regulatorio. No hay cron fijo.
- **Queries dinámicos:** El PM modifica los queries según lo que hay en KB, noticias recientes, y señales del entorno. Si un query no funciona, cámbialo — no lo repitas.
- **Sin castigo, con progreso:** Si no hay novedades, está bien. Pero el PM debe demostrar que intentó algo diferente, no que repitió lo mismo.
- **Trigger override:** Si algo rompe el ciclo (ej: noticia grande entre revisiones), el PM actúa inmediatamente sin esperar `next_review`.
- **Sugerir, no solo reportar:** Cuando algo no avanza, el PM propone alternativas concretas — otra fuente, otro ángulo, reclasificar el target, buscar por autor en vez de por tema.

---

## 19. KB QUEUE PROCESSING — triaje de colas desde el PM

El PM puede procesar las colas de staging (PerplexityQueue, AcademicQueue, NewsResults) sin necesidad de una sesión KB separada. El protocolo de evaluación vive en `SKILL-KB-current.md` — el PM lo lee del repo cada vez que necesita procesar colas.

### Cuándo ejecutar

- Cuando el bootstrap incluye `update all`
- Cuando el usuario pide explícitamente procesar colas
- Cuando el paso 7 del Session Startup reporta colas con >100 items acumulados y no hay sesión KB reciente

### Protocolo

1. **Leer el skill de evaluación del repo:**
```
Read file: thesis-repo/phd-kb/docs/SKILL-KB-current.md
```
Secciones relevantes: "Evaluation — How to Score Every Result", "Write to NewsLog", "Supabase — Primary Data Store", "Known Behaviors and Bugs". No memorizar — leer cada vez para heredar cambios que KB haya hecho.

2. **Seguir los batch limits del SKILL-KB:**

| Cola | Límite por sesión |
|------|------------------|
| PerplexityQueue | 50 items |
| AcademicQueue | 40 items |
| NewsResults | 40 items |
| Full update | 30 + 20 + 20 |

3. **Flujo por item:** Leer synthesis/abstract → evaluar (importance, capa, chapters, thesis_relevance) → escribir batch a Supabase → verificación ligera a Sheet → marcar status en cola de origen → siguiente batch.

4. **Conexiones:** Usa las mismas de §8 (SUPABASE_URL, SUPABASE_KEY, SHEET_API).

5. **Prevention Rules relevantes:** PR-005 (URL como lookup, no ID), PR-006 (promoteToNewsLog para staging→NewsLog), PR-010 (NewsResults mismo patrón).

### Lo que NO hace el PM al procesar colas

- No modifica index.html
- No modifica SKILL-KB
- No crea nuevos queries en la Queries tab (eso sí requiere sesión KB o coordinación)

Scripts GAS sí se pueden modificar si el triaje lo requiere (bug fix, nuevo campo, etc.) — despliegue vía CLASP (§20).

### Reporting

Al terminar, reportar en el formato del SKILL-KB: "X promoted, Y discarded, Z held. ALTA items: [lista]." Incluir conteo en el SessionLog y en CHANGELOG.md.

---

## 20. CLASP — DESPLIEGUE DE SCRIPTS GAS

Los scripts de Google Apps Script se gestionan como código fuente en `phd-kb/gas/`. El despliegue es automático vía GitHub Actions.

**Flujo:** editar `.js` en `phd-kb/gas/` → commit + push a `main` → GitHub Actions ejecuta `clasp push` → crea versión → actualiza deployment activo (~26s, sin intervención).

**Documentación completa:** `phd-kb/docs/KB-SystemReference-v1.txt` § "CI/CD — Apps Script via clasp + GitHub Actions". Incluye componentes, triggers, reversión, y credenciales.

**Reglas para el PM:**
- Editar archivos en `phd-kb/gas/`, nunca en el editor de Apps Script directamente
- Commit messages descriptivos: `"WebApp vN: descripción del cambio"`
- Si el push falla (secret `CLASPRC_JSON` expirado), notificar al doctorando — no intentar arreglar credenciales OAuth
- Verificar deploy exitoso en GitHub Actions después del push

---

## 21. INTEGRACIÓN DE CONNECTORS (Claude.ai)

**Propósito:** Scholar Gateway, Consensus, y Google Calendar son herramientas que el PM integra en el flujo de trabajo de investigación cuando se identifican gaps de conocimiento específicos. No son pipelines independientes — sus resultados pasan por el **mismo pipeline de evaluación** que cualquier otra fuente (importance, capa, chapters, thesis_relevance).

### Decisión arquitectónica

Los connectors son **demand-driven**, no mandatory:
- Se usan cuando el PM identifica un gap en el KB que requiere búsqueda semántica específica (Scholar Gateway) o señales de consenso científico (Consensus)
- Los resultados se registran en `evaluated_items` exactamente como otros items (no tabla separada)
- El trabajo se rastrea como `pm_tasks` con `task_type='busqueda'` para visibilidad y follow-up
- Las queries se registran en la tab Queries del Sheet (usando `addQuery` action) con notación de origen (connector)
- Google Calendar se usa en dos momentos: (1) **session startup** para verificar deadlines/reuniones próximas, (2) **advisory follow-up** para detectar conflictos de scheduling

### Cuándo usar cada connector

**Scholar Gateway — Búsqueda semántica académica:**
- Usar cuando: El PM necesita papers académicos sobre un tema específico con relevancia demostrada a la tesis
- Ejemplo: "Gap en NIST AI RMF + regulatory frameworks → Scholar Gateway busca 'NIST Cybersecurity Framework adaptations federal agencies'"
- Señal de éxito: 5+ papers nuevos con clara relevancia a capítulos específicos (2, 4, 6)
- Límite: No ejecutar más de 3 búsquedas Scholar Gateway por sesión (risk de sobrecarga evaluativa)

**Consensus — Papers con señales de acuerdo científico:**
- Usar cuando: El PM necesita entender si existe consenso o desacuerdo en la comunidad académica sobre un tema
- Ejemplo: "¿Hay consenso sobre si NIST RMF es sufficient para AI Act compliance? → Consensus busca papers sobre NIST + AI Act con métricas de acuerdo"
- Señal de éxito: Papers con acuerdo signal > 0.65 en el tema target
- Regla: Consensus es complementario a Scholar Gateway, no sustitutivo

**Google Calendar — Deadlines y reuniones:**
- **Session startup:** Leer calendar para próximas 30 días → notar deadlines que impacten roadmap de lectura (defensa de tesis, revisión de director, hito administrativo)
- **Advisory follow-up (§18):** Revisar calendar cuando se ejecutan advisories con `next_review` próxima → detectar conflictos de scheduling (ej: advisory review sale el mismo día que reunión con director)
- Crear eventos: **Solo con confirmación explícita del doctorando** (nunca automático)

### Workflow Scholar Gateway

```
1. PM identifica gap → notar en SESSION_LOG con descripción

2. Crear pm_task:
   INSERT INTO pm_tasks (
     title, description, task_type, due_date, review_status
   ) VALUES (
     'Scholar Gateway: [tema]',
     '[Scholar Gateway] Gap: [qué falta]. Query: "[search term]". Chapters: [N]. Expectation: [what we hope to find].',
     'busqueda',
     NULL,
     'none'
   );

3. Ejecutar búsqueda en Scholar Gateway → guardar resultados temporalmente

4. Evaluar CADA paper → pipeline completo:
   a. Leer abstract + first 2 pages
   b. Asignar: importance (ALTA/MEDIA/BAJA), capa, chapters (array), thesis_relevance
   c. Si relevancia demostrada → insertar en evaluated_items
   d. Si no relevante → loguear en SESSION_LOG con razón de rechazo

5. Registrar query en Queries tab:
   - Usar addQuery(query, source, result_count, notes)
   - Notes field: "[Scholar Gateway] Gap: [gap]. Found: N items, M evaluated, K inserted."

6. Actualizar pm_task:
   UPDATE pm_tasks
...
**Regla crítica:** No insertar resultados Scholar Gateway en `evaluated_items` sin pasar por evaluación completa. Cualquier paper que no pase capa + chapters + thesis_relevance no entra en el KB.

### Workflow Consensus

```
1. PM decides: ¿necesitamos señales de consenso en tema X?
...
2. Crear pm_task:
   INSERT INTO pm_tasks (
     ...
     'Consensus: [tema]',
     '[Consensus] Question: [scientific question]. Topic: [AI Act + NIST + federal agencies]. Min agreement signal: 0.60.',
     'busqueda',
     NULL,
     'none'
   );

3. Ejecutar búsqueda en Consensus → capturar:
   - Papers encontrados
   - Agreement signals (% científicos de acuerdo)
   - Direcciones del desacuerdo (si < 1.0)

4. Analizar resultados:
   a. Si agreement > 0.65: papers agregan solidez a argumento → ALTA importance por defecto
   b. Si agreement 0.40-0.65: existe tensión genuina → MEDIA importance, documentar ambos lados en thesis_relevance
   c. Si agreement < 0.40: desacuerdo fundamental → BAJA o no evaluar (depends contexto)

5. Insertar papers en evaluated_items (misma evaluación que Scholar Gateway)

6. Registrar query:
   addQuery(query, source, result_count, '[Consensus] Q: [question]. Agreement range: [min-max]. Found: N items.')

7. Actualizar pm_task → completed
```

**Nota:** Consensus es poderoso para tesis porque documenta estado del arte AND tensiones. Usar para capas 1 (marco) y 2 (RIA contexto).

### Integración Google Calendar

**Session Startup — paso 6b.1 (antes de Connector task pickup):**
```
1. Leer calendar próximos 30 días
2. Notar fechas críticas: hitos administrativos, reuniones con director, deadlines internos
3. Reportar en readiness: "Calendar: 3 events próximas (director meeting 2026-04-15, thesis registration deadline 2026-05-30)"
4. Si evento relevante impacta plan actual → alertar al doctorando
```

**Advisory Follow-up (§18 revisión):**
```
1. Cuando ejecutando advisory con next_review due:
   - Leer advisory.next_review, advisory.search_queries
   - Revisar calendar 7 días before/after next_review
   - Si hay conflicto (ej: reunion con director mismo día) → notar en check_notes:
     "Rescheduled: director meeting on 2026-04-15. Advisory checks moved to 2026-04-16."
   - Update next_review si necesario
```

**Crear eventos (solo con confirmación doctorando):**
```
Never insert Calendar events automatically. Solo después de:
1. PM propone: "¿Crear evento 'KB sweep' para 2026-03-25 09:00?"
2. Doctorando confirma explícitamente: "Yes" / "Sí"
3. Solo THEN insert event
```

Regla: **Google Calendar es read-only por defecto. Write require confirmation.**

### Task lifecycle para connector work

**Estado: pending (creado en startup/sesión):**
- pm_task existe con `done=false`
- Description documenta qué se busca y por qué
- Due_date puede ser NULL (demand-driven) o future date si es priority

**Estado: execution (durante sesión):**
- PM ejecuta búsqueda en Scholar Gateway / Consensus
- Resultados evaluados item-by-item
- Cada item insert en evaluated_items con `source_pipeline` = 'scholar_gateway' o 'consensus'
- Query registrado en Sheet (addQuery con connector notation)

**Estado: completed:**
- pm_task.review_status = 'completed'
- pm_task.completed_at = CURRENT_TIMESTAMP
- Si al cierre de sesión hay gaps identificados pero sin tiempo para ejecutar → pm_task queda pending (pickup en próxima sesión, paso 6b)

**Estado: deferred (si PM decides no ejecutar):**
- pm_task.review_status = 'deferred'
- review_notes documenta razón: "Deferred: context limit, priority lower than KB sweep"
- El doctorando puede retomar o descartar en próxima sesión startup

### Registro de queries en Sheet Queries tab

Todas las búsquedas Scholar Gateway / Consensus se registran via `addQuery` action (misma función que KB usa):

```
addQuery({
  query: "[exact search term used]",
  source: "scholar_gateway" OR "consensus",
  result_count: [N papers found],
  inserted_count: [M papers evaluated and added to KB],
  notes: "[Scholar Gateway] Gap: Ch4 NIST RMF + regulatory adapatation. Found 12, evaluated 10, inserted 8. Rejection reasons: 2 out of scope (generic AI ethics), 2 too early (pre-2020)."
});
```

**Propósito:** Queries tab actúa como audit trail de connector work. Permite:
- Revisar qué gaps se han searcheado (evitar duplicación)
- Evaluar eficiencia (how many searches → how many items evaluated → how many KB entries)
- Detectar patrones (ej: Scholar Gateway solo añade 60% de resultados → maybe refine search strategy)

### Rules no-negociables

1. **Evaluación completa:** Todo resultado de connector DEBE pasar pipeline evaluación (importance, capa, chapters, thesis_relevance). No shortcuts.

2. **Create task first:** No insertar resultado en evaluated_items sin pm_task que documenta QIÉN solicitó la búsqueda y POR QUÉ. Trazabilidad es obligatoria.

3. **Source pipeline:** Marcar siempre `source_pipeline = 'scholar_gateway'` o `'consensus'` en evaluated_items. Permite filtered queries después (ej: "show me all Scholar Gateway items", "what % of ALTA items came from Consensus").

4. **Query registration:** Cada búsqueda connector → query registration en Sheet. No silent executions.

5. **Google Calendar read-only default:** Revisar sí; crear/modificar/borrar eventos solo con confirmación doctorando (explicit in chat).

6. **Advisory scheduling:** Si advisory con next_review próxima coincide con evento calendar importante → reschedule advisory (notar en check_notes), **nunca** crear conflicto silenciosamente.

7. **Session reporting:** Si connector work fue ejecutado durante sesión → incluir en Session Closing Protocol (paso 7b). Si gaps identificados pero sin tiempo para ejecutar → crear pm_task (deferred) para pickup próxima sesión.

### Notas técnicas

- `source_pipeline` valores 'scholar_gateway' y 'consensus' ya están definidos en SKILL-KB v19 (ver KB evaluation pipeline)
- pm_tasks table ya existe en Supabase (usado por KB + PM)
- Google Calendar API disponible vía OAuth (requiere user auth, no automation)
- Si doctorando NO tiene Scholar Gateway / Consensus configurado en Claude.ai → notificar que el PM no puede ejecutar esa búsqueda
...
| v14 | 2026-03-22 | **chapters + protecciones de datos.** Columna `chapters` (int[]) implementada y poblada en 1,495 items (keyword heuristics). §8 actualizado con mapeo de capítulos, queries de consulta, y matriz de cobertura. Protecciones: trigger auto-corrección en Supabase (corrige en vez de rechazar), health check diario (pg_cron), corrections log. Pipeline documentada en `phd-kb/docs/PIPELINE-ARCHITECTURE.md`. §8 "TASK futuro" reemplazado con documentación de implementación. |
| v15 | 2026-03-22 | **Unificación canónica de capítulos.** Detectada inconsistencia triple: SKILL-PM, chapter_sections, y dashboard tenían estructuras de capítulos distintas. Resolución: (1) chapter_sections reescrita como fuente única de verdad (7 caps, 29 secciones), (2) evaluated_items.chapters remapeado (old 2→4, 3→2, 4→3) en 1,495 items, (3) vista `chapter_coverage` creada (KB + writing + reading en una query), (4) dashboard chapterCoverage ahora dinámico desde vista (eliminado hardcode de 8 caps), (5) reading_plan.chapter_ids (int[]) añadido y poblado (33 filas, texto→array con remapeo), (6) dashboard lecturas usan chapter_ids + getChapterMap() dinámico, (7) trigger documentado correctamente como validación estricta (no auto-corrección). §8 reescrito con estructura canónica y regla de fuente única. |
| v16 | 2026-03-22 | **Limpieza KB + estructura híbrida + reasignación.** (1) KB limpiada: 1,495→1,332 registros (163 duplicados fusionados con alt URLs preservadas, 27 author-names retitulados, 19 capas normalizadas, 2 noticias reclasificadas). (2) Estructura híbrida: chapter_sections reescrita (32 secciones): Ch2=Marco teórico + estado cuestión, Ch3=RIA (dedicado), Ch4=NIST AI RMF (dedicado), Metodología integrada en Ch2. (3) Reasignación completa: clasificador v2 (title-first, max 2 caps/item). Resultado: 94.4% single-chapter, 0 items con 3+ caps. (4) reading_plan.chapter_ids corregido para nueva numeración. (5) Milestone #6 actualizado. (6) kb_cleanup_log creada con 348+ log entries documentando cada decisión. §8 tabla de capítulos actualizada. |
| v17 | 2026-03-23 | **Infraestructura de integridad.** (1) Trigger actualizado: valida chapters entre 2-6, max 3 por item (primario/secundario/terciario), array posicional (primer elemento = cap primario). (2) Health check reescrito: eliminado falso positivo de chapters={4}, añadidos checks `invalid_chapter_nums` y `triple_chapter_items`. (3) GIN index en `chapters` + B-tree en `importance`. (4) 27 items sin language→'en', 2 sin content_type→'regulation'. (5) Dashboard: removido CHAPTER_TITLE_OVERRIDE que forzaba "Estado del Arte", bibliografía proyectada actualizada para estructura híbrida, gaps con números post-limpieza, sw.js v9. |
| v18 | 2026-03-23 | **PM puede procesar colas KB.** §1 interfaz actualizada: PM escribe en `evaluated_items` al hacer triaje. §6 Session Startup paso 7 añadido: queue check con reporte de pendientes. §19 añadido: protocolo de triaje de colas desde el PM — lee `SKILL-KB-current.md` del repo para evaluación (no duplica), hereda cambios automáticamente si KB actualiza su skill. Batch limits, prevention rules relevantes, y scope de lo que el PM no toca (scripts GAS, index.html, SKILL-KB, Queries tab). |
| v19 | 2026-03-23 | **CLASP para PM.** §20 añadido: referencia al pipeline CLASP CI/CD — PM puede editar y desplegar scripts GAS (`phd-kb/gas/`) directamente via push a main. Docs completas en `KB-SystemReference-v1.txt` (no duplicadas). §1 interfaz actualizada: scripts GAS ahora compartido (ambos proyectos). §3 repo structure: `phd-kb/gas/` visible con `.clasp.json`, `scripts/` marcado como archivo histórico. §14 prohibición actualizada (solo index.html + SKILL-KB requieren coordinación). §19 restricción GAS eliminada — PM puede modificar scripts si el triaje lo requiere. |
| v20 | 2026-03-24 | **Connector integration.** §21 añadido: Scholar Gateway, Consensus, y Google Calendar integrados al flujo PM como herramientas mid-workflow. Resultados de connectors pasan por evaluación completa (misma que §19 triaje). Connector tasks se rastrean como `pm_tasks` (task_type='busqueda') y queries se registran en Queries tab del Sheet. §1 interfaz actualizada con connectors. §6 startup añade paso 6b (pickup connector tasks), closing añade paso 7b (queue connector tasks). §12 infraestructura actualizada. §14 prohibiciones actualizadas. Google Calendar: lectura de deadlines/reuniones en startup y advisory follow-up. |

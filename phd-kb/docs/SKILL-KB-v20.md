...
| 5 — Scholar Gateway | Claude.ai connector | — | direct to Supabase | `update scholar_gw` |
| 6 — Consensus | Claude.ai connector | — | direct to Supabase | `update consensus` |
...
**Pipelines 5-6 (Scholar Gateway, Consensus) are Claude.ai connectors** invoked on-demand by the PM when gap analysis or advisory follow-up identifies a need. They don't have GAS scripts or staging queues — results are evaluated inline and written directly to Supabase. The PM creates a `pm_task` (task_type='busqueda') before executing, and logs queries in the Queries tab.
...
| `update scholar_gw` | Scholar Gateway semantic search for academic papers (PM-initiated, gap-driven) |
| `update consensus` | Consensus search with scientific agreement signals (PM-initiated, gap-driven) |
...
## `update scholar_gw` — Scholar Gateway Connector Search

Uses the Claude.ai Scholar Gateway connector for semantic academic search. **PM-initiated only** — triggered when gap analysis, advisory follow-up, or reading plan review identifies a need for additional academic sources.

### Prerequisites

1. A `pm_task` exists documenting what gap this search addresses
2. Gap or need is documented (from §16 gap tracker, advisory follow-up, or explicit user request)

### Execution

1. Use Scholar Gateway to search for the identified topic/author/concept
2. For each result that meets the logging bar: evaluate fully using the standard evaluation protocol below
3. Write to Supabase with `source_pipeline='scholar_gateway'`
4. Write verification row to Sheet NewsLog
5. Log query in Queries tab: `action=addQuery` with `notes="[Scholar Gateway] [context]"`
6. Update the `pm_task` with results count and close if gap addressed

**Batch limit:** 20 items per session (connector searches are targeted, not bulk).

**When to prefer Scholar Gateway over existing pipelines:**
- Academic papers not indexed by OpenAlex/Semantic Scholar/arXiv (e.g., legal scholarship, Spanish-language)
- Semantic concept searches where keyword matching fails
- Author discovery beyond tracked scholars
- Cross-disciplinary sources (regulatory science, forensic science, procedural law)

---

## `update consensus` — Consensus Connector Search

Uses the Claude.ai Consensus connector for AI-powered paper search with scientific agreement signals. **PM-initiated only.**

### Execution

Same flow as Scholar Gateway, plus:
- **Agreement signals** from Consensus (e.g., "87% of papers agree") serve as a **quality indicator**, not as a thesis argument. They help prioritize which papers to read first but are never cited as evidence.
- Write to Supabase with `source_pipeline='consensus'`
- When both Scholar Gateway and Consensus find the same paper, prefer the version with richer metadata

**Batch limit:** 20 items per session.

**When to prefer Consensus:**
- Empirical questions where scientific agreement matters (e.g., "does algorithmic fairness improve with transparency?")
- Quickly gauging research landscape on a subtopic before deep dive
- Identifying contrarian or minority positions (low agreement = interesting for thesis argumentation)

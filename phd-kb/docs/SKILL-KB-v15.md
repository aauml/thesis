...
## Supabase Dual-Write (TASK-006)

**Since 2026-03-19**, every evaluated item must be written to **both** Google Sheets (NewsLog) **and** Supabase. The Sheet write is the existing flow. The Supabase write is an additional step after each successful Sheet write.
...
After every successful `append` or `promoteToNewsLog` to the Sheet, also POST to Supabase:
...
| Sheet field | Supabase field | Notes |
|-------------|---------------|-------|
| `id` | `sheet_id` | Original base64[:16] ID |
| `url` | `url` | Unique constraint — duplicates ignored |
| `starred` | `starred` | Boolean in Supabase (`true`/`false`), string in Sheet (`TRUE`/`""`) |
| — | `source_pipeline` | New field: `perplexity` \| `academic` \| `news` \| `claude` \| `scholar_gateway` \| `consensus` |
| — | `migrated_from_sheet` | `false` for new items (only `true` for backfill) |
| — | `embedding` | 384d vector (gte-small via Supabase AI). Generated by `generate-embeddings` edge function. Auto-populated post-write. |
...
If the Supabase write fails, **do not block the Sheet write**. Log the error in `notes` field (e.g., `"supabase_write_failed"`) and continue. The item can be backfilled later. The Sheet remains the primary record during the transition period.

### Querying Supabase

For `what did we find about [topic]?` and `show archive`, you can also query Supabase:
...
- **Auto-population:** After dual-writing a new item, the embedding field is left null. The edge function can be called to backfill. Future: trigger automatically via webhook or scheduled task.

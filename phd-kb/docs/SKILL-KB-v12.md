...
At the start of every session, request access to the user's Google Drive sync folder:

```
Request: ~/Google Drive
Mounted at: /sessions/.../mnt/GoogleDrive-ayalaax@gmail.com
My Drive symlink resolves to: /sessions/.../mnt/My Drive
```

The canonical AppScripts folder path in the VM is:

```
/sessions/.../mnt/My Drive/PhD/09_Sistema/phd-kb/AppScripts/
```

This folder syncs to Google Drive automatically. All file writes here are immediately available in Drive. **Always request this mount at session start** — without it, files can only be saved to outputs (which the user must manually upload).
...
1. **Save the complete updated script** directly to the Drive AppScripts folder:
   ```
   /sessions/.../mnt/My Drive/PhD/09_Sistema/phd-kb/AppScripts/ScriptName-vN.txt
   ```
   Never save patches or diffs — always the full file. This is the canonical location.
2. **Also save to outputs** (`/sessions/.../mnt/outputs/`) so the user gets a download link in chat.
3. The user will locate the file in the Drive folder and paste it manually into the GAS editor.
...
**Naming convention:** `ScriptName-vN.txt` where N increments from the previous highest version in the AppScripts folder. If no previous version exists, start at `v1`. Check the folder listing before naming.
...
2. **Create `SKILL-KB-vN+1.md`** in the Drive AppScripts folder with all changes applied. Current version is v12, so next update produces `SKILL-KB-v13.md`.
...
Request access to `~/Google Drive`. Verify the AppScripts folder is accessible at:
```
/sessions/.../mnt/My Drive/PhD/09_Sistema/phd-kb/AppScripts/
```
...
```
Ruta: /sessions/.../mnt/My Drive/PhD/09_Sistema/phd-kb/AppScripts/KB-LessonsLog.md
```
...
```
Ruta: /sessions/.../mnt/My Drive/PhD/09_Sistema/phd-kb/AppScripts/KB-PendingIssues.md
```
...
**This protocol takes 1 minute and prevents repeating known mistakes.**
...
Agregar entrada en el Registro de decisiones del PM Log si hubo cambios significativos. Hacer commit y push al repo.

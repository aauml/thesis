...
- Interfaz compartida: **Supabase** (primario, lectura + escritura en `reading_plan`, `pm_advisories`, `reading_conversations`, y `evaluated_items`) + Sheet API (staging queues + verificación)
- Regla: PM no modifica scripts GAS ni index.html sin coordinar con KB. KB no modifica dashboard.html. PM lee `SKILL-KB-current.md` del repo para el protocolo de evaluación — si KB actualiza ese archivo, PM hereda los cambios automáticamente.
